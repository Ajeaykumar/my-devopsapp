...
# 1. Generate a new private key locally
resource "tls_private_key" "app_key" {
  algorithm = "RSA"
  rsa_bits  = 4096
}

# 2. Create an AWS Key Pair using the public key from above
resource "aws_key_pair" "app_kp" {
  key_name   = "my-app-key-${random_id.suffix.hex}" # Unique key name to avoid conflicts
  public_key = tls_private_key.app_key.public_key_openssh
}

# 3. Output the private key content (so you can save it)
output "private_key" {
...
# Optional: A random suffix for the key name to ensure uniqueness
